...
<div class="container">
    <div class="row">
        <div class="col-sm-12">
            <div class="card">
                <h2 class="card-header pt-3 pb-2">房間管理</h2>
                <div class="form-group pt-4 px-3 m-0">
                    <a href="{{route('rooms.create')}}" class="btn btn-success">新增房間</a>
                </div>
                <hr>
                <div class="card-body">
                        ...
                        <tbody>
                            @foreach ($rooms as $room)
                                <tr>
                                    ...
                                    <td>{{$room->name}}</td>
                                    <td><img src="{{Storage::url($room->image_url)}}" alt="" width="200"></td>
                                    <td>
                                        <a href="{{route('rooms.edit',['room'=>$room->id])}}" class="btn btn-primary">編輯</a>                                    
                                        <button class="btn btn-danger delete-btn">刪除</button> 
                                        <form class="d-none" action="{{route('rooms.destroy',['room'=>$room->id])}}" method="post">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
